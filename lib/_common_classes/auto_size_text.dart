...
class AutoSizeGroup {
  ...
  AutoSizeGroup({this.fontSize: double.infinity});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, size) {
      ...
      if (widget.group != null) {
        ...
        tmpHistory.add(widget.group!.fontSize);

        if(tmpHistory.length > widget.group!._listeners.length)
          tmpHistory.removeAt(0);

        double smallestRepeatingVal = -1;
        bool smallestRepeated = false;
        for(double size in tmpHistory) {
          if (smallestRepeatingVal > size || smallestRepeatingVal == -1) {
            smallestRepeatingVal = size;
            smallestRepeated = false;
            continue;
          }

          if(smallestRepeatingVal == size)
            smallestRepeated = true;
        }

        if(smallestRepeated){
          fontSize = smallestRepeatingVal;
          widget.group!.fontSize = smallestRepeatingVal;
          tmpHistory.clear();
        }
