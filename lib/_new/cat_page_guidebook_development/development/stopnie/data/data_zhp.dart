...
import 'package:harcapp/_new/cat_page_guidebook_development/development/_sprawnosci/data/data_spraw_zhp_harc.dart';
...
class AdditionalTaskEmptyButtons extends StatelessWidget{

  final void Function()? onSelectTap;
  final void Function()? onNoteTap;
  final bool required;

  const AdditionalTaskEmptyButtons({
    this.onSelectTap,
    this.onNoteTap,
    this.required = true,
    super.key,
...
class ItemSlidable extends StatelessWidget{
  ...
  final void Function(BuildContext, bool)? onCompletedTap;
  final bool? completed;
  final bool completedCheckable;
  ...
  Widget build(BuildContext context) => Slidable(
    startActionPane: ActionPane(
      extentRatio: .25*(onCompletedTap==null?1:2),
      motion: const ScrollMotion(),
      children: [
        if(onCompletedTap != null)
          Expanded(
            child: Container(
              height: double.infinity,
              color: backgroundIcon_(context).withOpacity(2*backgroundIcon_(context).opacity),
              child: IgnorePointer(
                ignoring: !completedCheckable,
                child: Checkbox(
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Dimen.ICON_SIZE)),
                  value: completed,
                  onChanged: (value) => onCompletedTap?.call(context, value??false),
...
class NoteTile extends StatelessWidget{
  ...
  const NoteTile({super.key, this.note, this.index, this.onNoteChanged, this.padding = EdgeInsets.zero});

  bool get noteEmpty => note == null || note!.isEmpty;
  ...
  Future<void> openEditDialog(BuildContext context, {required String initText, void Function(String)? onChanged}){
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: GestureDetector(
                          ...
                          child: Container( // This has to be here in order for the GestureDetector to work.
...
class SprawSelectedListWidgetState extends State<SprawSelectedListWidget>{
  ...
  String get code => RankZHPSim2022Templ.extSprawCode;
  int get count => widget.count;
  int? get reqCount => widget.reqCount;
  Color? get backgroundColor => widget.backgroundColor;
  Color? get rankColor => widget.rankColor;
  ...
  Widget build(BuildContext context) => Material(
      ...
      child: ListView.separated(
        ...
        shrinkWrap: true,
        separatorBuilder: (context, index) => const SizedBox(height: Dimen.defMarg/3),
          ...
          Widget emptyButtons = SizedBox(
            ...
            child: AdditionalTaskEmptyButtons(
              ...
              onSelectTap: () async {
                ...
                RankZHPSim2022Templ.setExtText(rankId, code, index, SprawSelectedListWidget.samplePrefix + selectedSpraw.uniqName);
                setState(() {});
              },
              onNoteTap: (){
          ...
          if (extText == null && enabled)
            ...
            return SizedBox(
                ...
                child: Padding(
                  ...
                  child: Text(
                    'Brak wybranej sprawności ${index+1}.',
                    style: AppTextStyle(
                      fontSize: Dimen.TEXT_SIZE_BIG,
            ...
            return ItemSlidable(
              ...
              onCompletedTap: (context, value){
                ...
                onCheckChanged?.call(index, value);
              },
              completedCheckable: checkable,
              completed: RankZHPSim2022Templ.getExtChecked(rankId, code, index),
              child: SizedBox(
                height: itemHeight,
                child: NoteTile(
                  note: extText.substring(SprawSelectedListWidget.customPrefix.length),
                  index: index,
                  padding: const EdgeInsets.symmetric(horizontal: Dimen.SIDE_MARG),
                  onNoteChanged: (text) async {
                    text = SprawSelectedListWidget.customPrefix + text;
                    setState(() => extText = text);
                    await RankZHPSim2022Templ.setExtText(rankId, code, index, text);
                  },
                ),
              ),
            );
            ...
            return ItemSlidable(
                ...
                child: SizedBox(
                  ...
                  child: SprawTileTemplateWidget(
                    ...
                    trailing: SprawTileProgressWidget(spraw: spraw),
                  ),
                )
            );
          else
...
class TropSelectedListWidget extends StatefulWidget{
  ...
  final Color? rankColor;
  final void Function(int, bool)? onCheckChanged;
  final void Function()? onTropStateChanged;
  final bool checkVisible;
  final bool checkable;
  final bool enabled;

  const TropSelectedListWidget(
      this.rankId,
      this.name,
      { required this.count,
        this.reqCount,
  ...
  @override
  State<StatefulWidget> createState() => TropSelectedListWidgetState();

}

class TropSelectedListWidgetState extends State<TropSelectedListWidget>{
  ...
  String get rankId => widget.rankId;
  String get name => widget.name;
  String get code => RankZHPSim2022Templ.extTropCode;
  int get count => widget.count;
  int? get reqCount => widget.reqCount;
  Color? get backgroundColor => widget.backgroundColor;
  Color? get rankColor => widget.rankColor;
  void Function(int, bool)? get onCheckChanged => widget.onCheckChanged;
  void Function()? get onTropStateChanged => widget.onTropStateChanged;
  bool get checkVisible => widget.checkVisible;
  bool get checkable => widget.checkable;
  bool get enabled => widget.enabled;
  ...
  void initState() {
    ...
    for(int i=0; i<count; i++) {
      ...
      String extText = RankZHPSim2022Templ.getExtText(rankId, code, i)??'';
      if(extText.contains(TropSelectedListWidget.customPrefix))
        extText = extText.substring(TropSelectedListWidget.customPrefix.length);
  ...
  Widget build(BuildContext context) => Material(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index){
          ...
          else if (extText!.startsWith(TropSelectedListWidget.customPrefix))
            return ItemSlidable(
              onRemoveTap: (context) async {
                String? oldExtText = RankZHPSim2022Templ.getExtText(rankId, code, index)??'';
                await RankZHPSim2022Templ.removeExtText(rankId, code, index);
              ...
              child: SizedBox(
                height: itemHeight,
                child: NoteTile(
                  note: extText.substring(TropSelectedListWidget.customPrefix.length),
                  index: index,
                  padding: const EdgeInsets.symmetric(horizontal: Dimen.SIDE_MARG),
                  onNoteChanged: (text) async {
                    text = TropSelectedListWidget.customPrefix + text;
                    setState(() => extText = text);
                    await RankZHPSim2022Templ.setExtText(rankId, code, index, text);
                  },
                ),
...
class WyzwanieSelectedListWidget extends StatefulWidget{
  ...
  final Color? rankColor;
  final void Function(int, bool)? onCheckChanged;
  final void Function()? onWyzwanieStateChanged;
  final bool checkVisible;
  final bool checkable;
  final bool enabled;

  const WyzwanieSelectedListWidget(
      this.rankId,
      this.name,
      { required this.count,
        this.reqCount,
  ...
  @override
  State<StatefulWidget> createState() => WyzwanieSelectedListWidgetState();

}

class WyzwanieSelectedListWidgetState extends State<WyzwanieSelectedListWidget>{
  ...
  String get rankId => widget.rankId;
  String get name => widget.name;
  String get code => RankZHPSim2022Templ.extWyzwCode;
  int get count => widget.count;
  int? get reqCount => widget.reqCount;
  Color? get backgroundColor => widget.backgroundColor;
  Color? get rankColor => widget.rankColor;
  void Function(int, bool)? get onCheckChanged => widget.onCheckChanged;
  void Function()? get onWyzwanieStateChanged => widget.onWyzwanieStateChanged;
  bool get checkVisible => widget.checkVisible;
  bool get checkable => widget.checkable;
  bool get enabled => widget.enabled;
        ...
        itemBuilder: (context, index){
          ...
          Widget emptyButtons = SizedBox(
            ...
            child: AdditionalTaskEmptyButtons(
              required: index < (reqCount??count),
              onSelectTap: () async {
                showAppToast(context, text: 'Ta funkcja jeszcze nie działa');
              },
              onNoteTap: (){
                RankZHPSim2022Templ.setExtText(rankId, code, index, SprawSelectedListWidget.customPrefix);
                setState(() {});
          ...
          if (extText == null && enabled)
            ...
            return SizedBox(
                ...
                child: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: Dimen.SIDE_MARG),
                  child: Text(
                    'Brak wybranego wyzwania ${index+1}.',
                    style: AppTextStyle(
                        fontSize: Dimen.TEXT_SIZE_BIG,
                        fontStyle: FontStyle.italic,
                        color: hintEnab_(context)
          ...
          groupData: [
            RankGroupData(
                ...
                taskData: [
                  ...
                  const RankTaskData(text: 'Poznałem najważniejsze tradycje i obrzę- dy swojej drużyny. Potrafię stanąć w postawie zasadniczej i swobodnej oraz ustawić się w szeregu. Wyka- załem się tą wiedzą i umiejętnościami uczestnicząc w obrzędach i apelu drużyny.'),
                ]
            ),
            RankGroupData(
              title: 'Obozownictwo',
              ...
              taskData: [
                const RankTaskData(text: 'Wziąłem udział w obozie lub biwaku pod namiotami. Poznałem zasady bezpieczeństwa i przestrzegałem ich. Po obozowisku nie zostawiłem żadnego śladu.'),
                const RankTaskData(text: 'Wyznaczyłem strony świata za pomocą dwóch wybranych sposobów.'),
                const RankTaskData(text: 'Znam i w różnych sytuacjach zastosowałem co najmniej pięć węzłów.'),
              ],
            ),
            RankGroupData(
              title: 'Ratownictwo',
            ...
            RankGroupData(
              ...
              taskData: [
                const RankTaskData(text: 'Przygotowałem pod nadzorem rodzica śniadanie lub kolację dla domowników, w tym jedno wybrane danie na ciepło, np.: owsiankę, jajecznicę, parówki na gorąco.'),
              ],
            ),
            RankGroupData(
              title: 'Zdrowie',
              ...
              taskData: [
                const RankTaskData(text: 'Wziąłem udział w wycieczce pieszej, rowerowej lub kajakowej trwającej około 4-5 godzin. Samodzielnie spakowałem plecak uwzględniając zdrowe przekąski, wodę, okrycie na słońce i deszcz, krem z filtrem itd.'),
              ],
            ),
            RankGroupData(
              title: 'Świat cyfrowy',
              ...
              taskData: [
                const RankTaskData(text: 'Zapoznałem się z zasadami BHP podczas pracy przy komputerze. Porównałem je z moimi przyzwyczajeniami i skorygowałem (w razie potrzeby) swoje nawyki.'),
              ],
            ),
            RankGroupData(
              title: 'Przyroda',
            ...
            RankGroupData(
              ...
              taskData: [
                const RankTaskData(text: 'Zaoszczędziłem pieniądze na wybrany przez siebie cel, np.: zakup produktu lub usługi, opłacenie składki członkowskiej.'),
                const RankTaskData(text: 'Sporządziłem wraz z rodzicami listę potrzebnych produktów, porównałem ich ceny oraz odmiany w różnych sklepach. Podczas dokonywania zakupów nie przekroczyłem założonego budżetu.')
              ],
            ),
            RankGroupData(
                ...
                taskData: [
                  const RankTaskData(text: 'Utrzymuję porządek w swoim pokoju.'),
                  const RankTaskData(text: 'Mocno i estetycznie przyszyłem łatę lub naszywkę oraz guzik.')
                ]
            )
          ]
      )
      ...
      RankCatData(
          ...
          groupData: [
            RankGroupData(
              ...
              icon: RankData.iconTozsHarcerska,
              taskData: [
                const RankTaskData(text: 'Udokumentowałem w kronice drużyny (lub na stronie internetowej lub fanpage’u) jedno wydarzenie z jej życia.'),
              ],
            ),
            RankGroupData(
              title: 'Obozownictwo',
              icon: RankData.iconObozow,
              taskData: [
                const RankTaskData(text: 'Zachowując zasady bezpieczeństwa, rozpaliłem ogień na łonie natury z użyciem wyłącznie naturalnych materiałów (nie dotyczy zapałek/zapalniczki); po zgaszeniu doprowadziłem miejsce do pierwotnego stanu. Uczestniczyłem w przygotowaniu złożonego posiłku na ognisku.'),
                const RankTaskData(text: 'Wraz z zastępem rozstawiłem namiot i zbudowałem przydatne urządzenie obozowe (np. wyposażenie namiotu), przez cały obóz utrzymałem porządek w swoich rzeczach.'),
                const RankTaskData(text: 'Pełniłem służbę wartowniczą na obozie lub biwaku.'),
              ],
            ),
            RankGroupData(
              title: 'Ratownictwo',
              icon: RankData.iconPierwszaPomoc,
              taskData: [
                const RankTaskData(text: 'W sytuacji rzeczywistej lub symulowanej przy użyciu własnej apteczki prawidłowo postąpiłem przy krwotokach:'
                    '\n\n• udzieliłempierwszejpomocywprzypadku krwotoku z nosa,'
                    '\n• założyłem opatrunek uciskowy w przypadku krwotoku z rany.'),
                const RankTaskData(text: 'Udzieliłem pierwszej pomocy w przypadku ukąszenia przez owady.'),
              ],
            ),
            RankGroupData(
              title: 'Przyroda',
              icon: RankData.iconEkologia,
              taskData: [
                const RankTaskData(text: 'Korzystałem wyłącznie z wielorazowych naczyń i sztućców na biwakach, imprezach szkolnych i harcerskich przez minimum trzy miesiące.'),
                const RankTaskData(text: 'Przez okres trwania próby opiekowałem się zwierzęciem lub rośliną.'),
              ],
            ),
            RankGroupData(
              title: 'Ekonomia',
              icon: RankData.iconEkonomia,
              taskData: [
                const RankTaskData(text: 'Wykonałem jedno z poniższych zadań:'
            ...
            RankGroupData(
              ...
              icon: RankData.iconZywienie,
              taskData: [
                const RankTaskData(text: 'Nauczyłem się przygotowywać ulubioną potrawę z rodzinnego przepisu.'),
              ],
            ),
            RankGroupData(
              title: 'Zdrowie',
              icon: RankData.iconZdrowie,
              taskData: [
                const RankTaskData(text: 'Wziąłem udział w całodziennej wycieczce pieszej, rowerowej lub kajakowej. Dowiedziałem się jak i przygotowałem na niej obiad niewymagający gotowania.'),
                const RankTaskData(text: 'Przez okres trwania próby ćwiczyłem wybrany sport lub dziedzinę sprawności fizycznej (np.: szybkość, skoczność, celność, utrzymywanie równowagi). Zapisałem wynik osiągany na początku (np. czas na 100 m) i poprawiłem go na koniec próby.')
              ],
            ),
            RankGroupData(
              title: 'Zaradność życiowa',
              icon: RankData.iconZaradnosc,
              taskData: [
                const RankTaskData(text: 'Mam stały obowiązek domowy związany z obsługą urządzeń technicznych i wywiązuję się z niego.')
              ],
            ),
            RankGroupData(
              title: 'Świat cyfrowy',
              icon: RankData.iconSwiatCyfrowy,
              taskData: [
                const RankTaskData(text: 'Zapoznałem się z netykietą, czyli zbiorem zasad zachowania się w Internecie. Stosuję je podczas codziennego użytkowania sieci.')
              ],
            ),
      ...
      RankCatData(
          ...
          groupData: [
            RankGroupData(
              ...
              icon: RankData.iconTozsHarcerska,
              taskData: [
                const RankTaskData(text: 'Odwiedziłem jedno z miejsc związanych z ważnymi dla harcerstwa wydarzeniami.'),
                const RankTaskData(text: 'Poznałem życiorysy ośmiu osób zasłużonych dla rozwoju ruchu harcerskiego. O dwóch spośród nich opowiedziałem podczas zbiórki zastępu lub drużyny.'),
              ],
            ),
            RankGroupData(
              title: 'Obozownictwo',
              icon: RankData.iconObozow,
              taskData: [
                const RankTaskData(text: 'Wziąłem udział w kilkudniowym rajdzie lub wyprawie z nocowaniem na łonie przyrody, podczas którego minimalizowałem swój wpływ na środowisko (odpowiednio gospodarując wodą i odpadami, przygotowując nocleg, zabezpieczając ognisko).'),
                const RankTaskData(text: 'Dbam o sprzęt drużyny lub szczepu. Brałem udział w jego konserwacji.'),
                const RankTaskData(text: 'Samodzielnie wykonałem obrzędowy stos ogniskowy.')
              ],
            ),
            RankGroupData(
              title: 'Ratownictwo',
            ...
            RankGroupData(
              ...
              icon: RankData.iconZdrowie,
              taskData: [
                const RankTaskData(text: 'Przez dwa tygodnie nie korzystałem z telefonu na godzinę przed snem dokumentując to (np. w aplikacji Forest) oraz dowiedziałem się, dlaczego używanie telefonu przed snem jest niekorzystne dla zdrowia.'),
                const RankTaskData(text: 'Przez okres trwania próby minimum raz w tygodniu uprawiałem swoją ulubioną aktywność fizyczną.')
              ],
            ),
            RankGroupData(
              title: 'Przyroda',
              icon: RankData.iconEkologia,
              taskData: [
                const RankTaskData(text: 'Wziąłem udział w kilkudniowej wyprawie z nocowaniem na łonie przyrody. Nauczyłem się odpowiednio załatwić swoje potrzeby fizjologiczne będąc w terenie.'),
                const RankTaskData(text: 'Wiem, co to jest rezerwat przyrody, park narodowy i park krajobrazowy. W czasie harcerskich wędrówek odwiedziłem jeden z parków narodowych lub krajobrazowych.')
              ],
            ),
            RankGroupData(
              title: 'Świat cyfrowy',
              icon: RankData.iconSwiatCyfrowy,
              taskData: [
                const RankTaskData(text: 'Utworzyłem konto mailowe. Wykorzystuję podstawowe funkcje klienta pocztowego, m.in. ustawiłem podpis w mailu, przy wysyłaniu maili do wielu osób korzystam z opcji UDW.')
              ],
            ),
            RankGroupData(
              title: 'Ekonomia',
              icon: RankData.iconEkonomia,
              taskData: [
                const RankTaskData(text: 'Wykorzystując więcej niż jeden sposób, pozyskałem środki finansowe na wybrany przez siebie cel.')
              ],
            ),
            RankGroupData(
              title: 'Zaradność życiowa',
              icon: RankData.iconZaradnosc,
              taskData: [
                const RankTaskData(text: 'Zdobyłem nową umiejętność przydatną w gospodarstwie domowym. Rzetelnie wykonuję swoje obowiązki domowe.')
              ],
            ),
            RankGroupData(
              title: 'Żywienie',
              icon: RankData.iconZywienie,
              taskData: [
                const RankTaskData(text: 'Przygotowałem potrawę tradycyjną dla wybranego regionu Polski (np. proziaki z Podkarpacia, babkę ziemniaczaną z Podlasia).')
              ],
            ),
          ]
      )
      ...
      RankCatData(
          ...
          groupData: [
            RankGroupData(
              ...
              icon: RankData.iconTozsHarcerska,
              taskData: [
                const RankTaskData(text: 'Przeczytałem przynajmniej jedną książkę, która pogłębiła moją wiedzę o dziejach ruchu harcerskiego lub skautowego. Zaprezentowałem innym (w zastępie, drużynie) wybrane zagadnienie i znaczącą postać.'),
                const RankTaskData(text: 'Spotkałem członka władz mojego hufca. Dowiedziałem się jak oznacza się funkcje w hufcu, chorągwi i władzach naczelnych ZHP i jakie są zadania tych poziomów organizacji.'),
              ],
            ),
            RankGroupData(
              title: 'Obozownictwo',
              icon: RankData.iconObozow,
              taskData: [
                const RankTaskData(text: 'Zapoznałem młodszych harcerzy z zasadami pracy z narzędziami pionierskimi. Pokierowałem zastępem przy budowie prostego urządzenia obozowego.'),
                const RankTaskData(text: 'Podjąłem się funkcji oboźnego, kwatermistrza lub zaopatrzeniowca na wyjeździe drużyny.'),
              ],
            ),
            RankGroupData(
              title: 'Ratownictwo',
              icon: RankData.iconPierwszaPomoc,
              taskData: [
                const RankTaskData(text: 'W sytuacji rzeczywistej lub symulowanej rozpoznałem objawy zawału serca i udaru. Udzieliłem pierwszej pomocy poszkodowanemu.'),
                const RankTaskData(text: 'W sytuacji rzeczywistej lub symulowanej udzieliłem pierwszej pomocy w przypadku napadu duszności i zadławienia u osoby dorosłej.'),
                const RankTaskData(text: 'Współprowadziłem zbiórkę na temat pierwszej pomocy dla innych harcerzy.')
              ],
            ),
            RankGroupData(
              title: 'Zdrowie',
              icon: RankData.iconZdrowie,
              taskData: [
                const RankTaskData(text: 'Przez trzy miesiące spałem minimum po siedem godzin dziennie, dbając o to, aby przed snem się wyciszyć.'),
                const RankTaskData(text: 'Przez okres trwania próby minimum raz w tygodniu uprawiałem swoją ulubioną aktywność fizyczną. Dobrałem do niej odpowiednią dietę w porozumieniu z rodzicami lub dietetykiem.'),
              ],
            ),
            ...
            RankGroupData(
              ...
              icon: RankData.iconZaradnosc,
              taskData: [
                const RankTaskData(text: 'Wykonałem przedmiot użyteczny w działaniach drużyny lub szczepu.'),
              ],
            ),
            ...
            RankGroupData(
              ...
              taskData: [
                ...
                const RankTaskData(text: 'Poznałem teorię "globalnego ocieplenia" i rozumiem jego złożoność. Zrozumiałem konsekwencje pochopnych propozycji rozwiązań.'
                    '\n\nW trakcie trwania próby wyrobiłem w sobie nowy nawyk oszczędzania zasobów Ziemi.'),

                const RankTaskData(text: 'Opowiem o jednej dziedzinie specjalizacji potrzebnej do próby rozwiązania problemu "globalnego ocieplenia".'
            ...
            RankGroupData(
              ...
              icon: RankData.iconSwiatCyfrowy,
              taskData: [
                const RankTaskData(text: 'Wykonałem jedno z zadań:'
            ...
            RankGroupData(
              ...
              icon: RankData.iconZywienie,
              taskData: [
                const RankTaskData(text: 'Ugotowałem co najmniej po dwie różne zupy, drugie dania i desery (niekoniecznie jednego dnia) wykorzystując różne sposoby przygotowania, np.: pieczenie, smażenie, duszenie, gotowanie. Nakryłem do stołu z okazji uroczystego posiłku, korzystając z zastawy, sztućców, serwetek, wybranych elementów dekoracyjnych (np.: świece, kwiaty, obrus).'),
              ],
            ),
          ]
      )
      ...
      RankCatData(
          ...
          groupData: [
            RankGroupData(
              ...
              icon: RankData.iconTozsHarcerska,
              taskData: [
                const RankTaskData(text: 'Przekazał młodszym członkom swojego środowiska harcerskiego wiedzę o jego tradycjach w ciekawej formie.'),
              ],
            ),
            RankGroupData(
              title: 'Obozownictwo',
              icon: RankData.iconObozow,
              taskData: [
                const RankTaskData(text: 'Wziął udział w przygotowaniu obozu np.: planowaniu preliminarza, przygotowaniu sprzętu, organizacji transportu, zaplanowaniu zakupów żywieniowych. Poznał zasady prowadzenia gospodarczej dokumentacji obozu i pod nadzorem kwatermistrza pomógł ją prowadzić.'),
              ],
            ),
            RankGroupData(
              title: 'Ratownictwo',
            ...
            RankGroupData(
              ...
              icon: RankData.iconZdrowie,
              taskData: [
                const RankTaskData(text: 'Zaplanował i zorganizował długotrwały wysiłek fizyczny o charakterze wyczynu i dobrał dostosowaną do niego dietę. W miarę możliwości uzyskał dla swojego pomysłu akceptację profesjonalisty.'),
              ],
            ),
            RankGroupData(
              title: 'Przyroda',
              icon: RankData.iconEkologia,
              taskData: [
                const RankTaskData(text: 'Poznał podstawowe problemy związane z dostępem do wody pitnej (w tym jej niedoboru) na świecie i w Polsce. Poznał różne sposoby pozyskania lub uzdatniania wody pitnej w terenie (destylacja, filtry osobiste, filtracja i uzdatnianie chemiczne itp.). Podczas wędrówki zastosował jeden z nich.'),
                const RankTaskData(text: 'Zna i stosuje zasady ekologicznego obozowania. Wdraża w nie młodszych od siebie harcerzy, pomagając im w ekologicznym obozowaniu lub biwakowaniu.')
              ],
            ),
            RankGroupData(
              title: 'Świat cyfrowy',
              icon: RankData.iconSwiatCyfrowy,
              taskData: [
                const RankTaskData(text: 'Zna pojęcie tzw. "zniewagi" i uczestniczył w projekcie lub kampanii, która miała wpływ na ograniczenie tego zjawiska w sieci.'),
              ],
            ),
            RankGroupData(
              title: 'Ekonomia',
              icon: RankData.iconEkonomia,
              taskData: [
                const RankTaskData(text: 'Zidentyfikował swoje predyspozycje zawodowe korzystając z pomocy doradcy zawodowego lub wybranego kwestionariusza.'),
              ],
            ),
          ]
      )
