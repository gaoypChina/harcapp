...
class TropUsersPageState extends State<TropUsersPage>{
  ...
  Widget build(BuildContext context) => Consumer<CommunityManagersProvider>(
      builder: (context, prov, child){
        ...
        return UserListManagementLoadablePage<TropUser>(
            ...
            if(trop.myRole == TropRole.OWNER)
              IconButton(
                  ...
                  onPressed: () => showScrollBottomSheet(
                      context: context,
                      builder: (context) => AddUserBottomSheet(trop)
                  )
              )
          ],

          userCount: trop.assignedUsers.length + (trop.userCount??0),
          callReload: () async {
            await ApiTrop.getUsers(
              ...
              onSuccess: (observersPage){
                TropUser me = trop.loadedUsersMap[AccountData.key]!;
