...
class SettingsPage extends StatefulWidget {
  ...
  final SpiritPageState parent;
  const SettingsPage(this.parent);
...
class SettingsPageState extends State<SettingsPage> {
  ...
  void displayLocalCount() {
    ...
    for(Source source in itemsProv.sources) {
      int count = (ImageLoader.getLocalItems(shaPref, source)).length;
      savedLocally[source.code] = count;
      double size = (ImageLoader.getSavedSourceSize(source))/(1024*1024);
      savedLocallySize[source.code] = size;
    }
  ...
  void initState() {
    ...
    itemsProv = Provider.of<ItemsProvider>(widget.parent.context, listen: false);

    setState(() => displayLocalCount());
    super.initState();
  }
        ...
        slivers: [
          ...
          SliverList(delegate: SliverChildBuilderDelegate(
              (context, index) => ItemWidget(
                  this, itemsProv.sources[index],
                  savedLocally[itemsProv.sources[index].code],
                  savedLocallySize[itemsProv.sources[index].code]
              ),
            childCount: itemsProv.sources.length
          ))
        ],
      ),
    );
...
class ItemWidget extends StatefulWidget{
  ...
  const ItemWidget(this.parent, this.source, this.savedLocally, this.savedLocallySize);
...
class ItemWidgetState extends State<ItemWidget>{
        ...
        SizedBox(
          ...
          child: AppCard(
            ...
            onTap: (){
              ...
              parent.widget.parent.tryJumpToPage(0);
            },
            radius: AppCard.BIG_RADIUS,
            margin: EdgeInsets.all(Dimen.ICON_MARG),
            child: Stack(
              children: [

                Positioned.fill(
                  child: Image.asset('assets/images/stref_duch/' + SettingsPageState.thumbnails[source.code], fit: BoxFit.cover),
                ),
          ...
          child: Column(
            children: [
              ListTile(
                ...
                subtitle: Text('Zapisane lokalnie: ${widget.savedLocally==null?'...':widget.savedLocally}/${source.items.length}\t\t(${widget.savedLocallySize==null?'...':widget.savedLocallySize.toStringAsFixed(1)} MB)', style: hintStyle),
              ),
              Row(
                children: [

                  if(widget.savedLocally == source.items.length)
                    Padding(padding: EdgeInsets.all(Dimen.ICON_MARG), child: Icon(MdiIcons.check, color: accent_(context)))
                  else
                    AnimatedCrossFade(
                      firstChild: IconButton(icon: Icon(MdiIcons.downloadOutline, color: accent_(context)), onPressed: (){
                        download();
                      },),
                      secondChild: Stack(
                        children: [
  ...
  Future<void> download() async {
    ...
    for(int i=0; i<source.items.length; i++){
      ...
      await ImageLoader.downloadSaveImage(source.items[i], true);
...
class Settings{
  ...
  static bool get showInitMessage => shaPref.getBool(ShaPref.SHA_PREF_DUCHOWE_INIT_MESSAGE, false);
  static set showInitMessage(bool value) => shaPref.setBool(ShaPref.SHA_PREF_DUCHOWE_INIT_MESSAGE, value);

  static bool get saveLocally => shaPref.getBool(ShaPref.SHA_PREF_DUCHOWE_SAVE_LOCALLY, false);
